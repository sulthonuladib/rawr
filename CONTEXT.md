...
Last updated: 2026-09-11. Owner: main session. Status: Phase 0 foundation scaffolded, `pnpm i/check/test/build` + `docker compose config` green; `packages/domain` implemented (`pnpm --filter @rawr/domain check` green, NOT committed); `packages/config` implemented (`pnpm --filter @rawr/config check` green + runtime smoke green, NOT committed); `packages/observability` implemented (`pnpm --filter @rawr/observability check` green + runtime smoke green, NOT committed); `packages/coin-store` implemented (`pnpm --filter @rawr/coin-store check` green + pg-proxy fake-driver smoke green, NOT committed); `packages/messaging` implemented (`pnpm --filter @rawr/messaging check` green + runtime smoke green, NOT committed).
...
- [x] `packages/coin-store` (this session): `src/schema.ts` (4 snake_case `pgTable`s: `active_coins` cmc_id-pk + 30 legacy listing cols + `status`, `exchange_symbols` (exchange,cmc_id) pk, `orderbook_snapshots` + `opportunities` serial-pk with numeric prices/profit + tz timestamps; driver-agnostic `Db = PgDatabase<PgQueryResultHKT, typeof schema>` port), `src/active-coins.ts` (`findActiveById/listActive/upsertActive` via `onConflictDoUpdate` in tx dual-writing both tables/`setStatus`/`existsOnOther` SQL check/`toActiveCoin[toInsert]`), `src/market-store.ts` (`saveSnapshot/saveOpportunity` with numeric-profit guard/`listOpportunities` unexpired-only + `StoredOpportunity`). All `Effect` + `CoinNotFound|StoreUnavailable|InvalidCoinError`, rows parsed via domain Schemas, zero deletes (legacy `destroy` not ported), JSDoc on all exports. Added `@rawr/domain: workspace:*` dep + `exports` maps to `domain` + `coin-store` package.json (first cross-package import; `dist`-based resolution). `check` green + pg-proxy fake-driver smoke green (9 behavior + 9 SQL-shape assertions). NOT committed.
- [x] `packages/messaging` (this session): `BusEvent` union (`CoinUpdated | CrawlerReport`, disjoint `cmcId`/`market` keys, `parse/encodeBusEvent`), `Messaging` port (`publish` + scoped `subscribe(handler)`) with `MessagingError` TaggedError, `layerMemory` (`PubSub.bounded` 256/replay 50, backpressure, `forkScoped` + `Stream.runForEach`), `layerRabbitMq` skeleton over narrow `AmqpChannel` port (asserts 17-queue topology with `Effect.retry(reconnectSchedule)`, Schema encode/decode at edges, consumes `coin-updates` + `crawler-logs`), legacy-faithful topology (`ExchangeQueueNames` in `amqp.js` order, `MaxLengthPerQueue` 500/1000 verbatim, `durable: false` everywhere, `exchangeToQueue` mapping `upbitUsdt->upbit_usdt`), new `coin-updates` queue (no `maxLength` — lifecycle never dropped), `reconnectSchedule` (exponential 500ms cap 30s + jitter, replaces fixed 2s), `loadAmqpUrl` from `@rawr/config` (`Redacted` passthrough). Added `workspace:*` deps + `exports` maps (`messaging`, plus `config` + `observability` which lacked them; rebuilt stale `config` dist so `loadConfig` resolves). `check` green + 16-assertion runtime smoke green (topology, routing, roundtrip, poison-as-value, live memory-bus delivery of both events). NOT committed.
- [ ] Next: coin-admin-api + ingest-sender FiberMap (`@effect-service`)
